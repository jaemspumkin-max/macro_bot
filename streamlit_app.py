...
def run_analysis(weights=None, alpha_key=None):
    if weights:
        engine = MacroBiasEngine(weights=weights, alpha_vantage_key=alpha_key)
    else:
        engine = MacroBiasEngine(alpha_vantage_key=alpha_key)
...
try:
    with st.spinner("🔄 Fetching data from 4 sources..."):
        results = run_analysis(custom_weights, alpha_key if 'alpha_key' in locals() else None)
