...
with st.sidebar:
    ...
    if TRACKER_AVAILABLE:
        ...
        if len(history_df) < 5:
            ...
            with col1:
                if st.button("⚡ Quick Fill (10 days)", help="Faster - takes ~2 minutes", use_container_width=True):
                    progress_bar = st.progress(0)
                    status_text = st.empty()
                    
                    with st.spinner("⏳ Fetching last 10 days..."):
                        try:
                            successful = tracker.backfill_history(days=10, custom_weights=custom_weights if custom_weights else None)
                            progress_bar.progress(100)
                            status_text.success(f"✅ Backfilled {successful} days!")
                            time.sleep(2)
                            st.rerun()
            ...
            with col2:
                if st.button("📅 Full Fill (30 days)", help="Slower - takes ~5 minutes", use_container_width=True):
                    progress_bar = st.progress(0)
                    status_text = st.empty()
...
with col2:
    # Show current time
    ny_tz = timezone(timedelta(hours=-5))  # EST
    ny_time = datetime.now(ny_tz)
    st.markdown(f"**NY Time:** {ny_time.strftime('%H:%M ET')}")
with col3:
    # Data freshness indicator
    ny_hour = ny_time.hour
    if 9 <= ny_hour < 16:
        st.success("🟢 **Market Open**")
        st.caption("Live data updating")
    elif 4 <= ny_hour < 9:
        st.info("🔵 **Pre-Market**")
        st.caption(f"Opens at 9:30 AM ET")
    else:
        st.warning("⚪ **After Hours**")
        st.caption("Next: 9:30 AM ET")
...
if 'sources_used' in results:
    ...
    with st.expander("⏰ When Does Data Update?"):
        st.markdown("""
        ### Real-Time Data (During Market Hours: 9:30 AM - 4:00 PM ET)
        - **VIX Index** → Every minute
        - **SPY, QQQ, SMH** → Every minute
        - **HYG, AGG, GLD, USO** → Every minute
        - **DXY (Dollar)** → Every minute
        - **VXN (Nasdaq Vol)** → Every minute
        
        ### Daily Data (Updates Evening/Next Day)
        - **10Y Treasury Yield** → ~3-4 PM ET
        - **2Y Treasury Yield** → ~3-4 PM ET
        - **Credit Spreads** → ~5 PM ET
        
        ### Weekly/Monthly Data
        - **M2 Money Supply** → Monthly (typically Tuesday)
        
        ---
        
        **📍 You Are Currently Viewing:**
        - **Before Market Open (Pre 9:30 AM ET):** Previous day's closing data
        - **During Market Hours (9:30 AM - 4 PM ET):** Live real-time data
        - **After Market Close (Post 4 PM ET):** Today's closing data
        
        **🔄 Best Times to Check:**
        - **9:35 AM ET** → Fresh market open data
        - **12:00 PM ET** → Mid-day update
        - **4:05 PM ET** → Market close data
        - **Avoid:** 3:00 AM - 9:00 AM ET (stale data)
        """)
