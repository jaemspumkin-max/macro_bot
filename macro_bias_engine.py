...
class MacroDataFetcher:
    ...
    def fetch_fred_data(self, series_id, forward_fill=True):
        """
        Fetch data from FRED (Federal Reserve Economic Data).
        
        Args:
            series_id (str): FRED series identifier
            forward_fill (bool): Fill weekends/holidays with last known value
        ...
        try:
            # Extend date range to ensure we have recent data even on weekends
            extended_start = self.start_date - timedelta(days=30)
            data = pdr.DataReader(series_id, 'fred', extended_start, self.end_date)
            ...
            # Forward fill for weekends and holidays (use last known value)
            if forward_fill and len(series) > 0:
                series = series.ffill()
            
            # Drop remaining NaN values
            series = series.dropna()
            
            # Filter to our actual date range
            series = series[series.index >= self.start_date]
            
            if len(series) > 0:
                print(f"✅ Fetched {series_id}: {len(series)} data points, latest = {series.iloc[-1]:.2f}")
                return series
            else:
                return None
    ...
    def fetch_yahoo_data(self, ticker, forward_fill=True):
        """
        Fetch data from Yahoo Finance.
        
        Args:
            ticker (str): Yahoo Finance ticker symbol
            forward_fill (bool): Fill weekends with last known value
        ...
        try:
            # Add buffer to date range for weekends
            extended_start = self.start_date - timedelta(days=10)
            data = yf.download(ticker, start=extended_start, end=self.end_date, 
                             progress=False)
            ...
            if isinstance(data, pd.DataFrame) and not data.empty:
                ...
                # Forward fill for weekends (use Friday's close for Sat/Sun)
                if forward_fill and len(series) > 0:
                    series = series.ffill()
                ...
                if len(series) > 0:
                    print(f"✅ Fetched {ticker}: {len(series)} data points, latest = {series.iloc[-1]:.2f}")
                    return series
            return None
        except Exception as e:
